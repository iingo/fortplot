...
contains
    ...
    subroutine dispatch_histogram(data, bins, range, density, weights, cumulative, &
        ...
        !!
        !! Supports `range`, `weights`, and `cumulative` by preprocessing the
        !! sample array and post-processing the counts before forwarding to
        !! `fig%add_hist`. `histtype`, `orientation`, `stacked`, `log`, and
        !! `alpha` are accepted for matplotlib parity; they are currently
        !! stored as metadata and will influence rendering once the pipeline
        !! gains support (tracked via the referenced issues).
        use fortplot_figure_histogram, only: calculate_histogram_bins, &
                                             create_histogram_line_data
    ...
    subroutine finalise_histogram(alpha, histtype, stacked, log)
        ...
        ! histtype/stacked/log parameters are accepted for matplotlib parity
        ! but not yet rendered; noted silently to avoid warning noise.
